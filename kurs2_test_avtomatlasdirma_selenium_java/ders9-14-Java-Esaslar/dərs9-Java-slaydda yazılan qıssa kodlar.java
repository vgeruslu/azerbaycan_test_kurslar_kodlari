public class DeyişenNövleri {

    public static void main(String[] args) {
        // Tam ədəd növü
        int əhali = 10123456; // 10,123,456
				//-2,147,483,648 ilə 2,147,483,647 arasında tam ədəd

        // Ondalık nömrə növü
        double böyükDəqiqlikləMaaş = 2789.123456; 
		// Ondalık ədədlər üçün, dəqiqliyi 15 rəqəmə qədər

        // Mətn növü
        char hərf = 'A'; // Tək bir simvol saxlayır
        String mesaj = "Salam Azərbaycan!"; // Mətn sətiri

        // Məntiqi növ
        boolean azadliq = true; // true və ya false dəyəri ala bilər

        // Çap edirik
        System.out.println("Əhali: " + əhaliMilliyonlarla);
        System.out.println("Böyük dəqiqliklə maaş: " + böyükDəqiqlikləMaas);
        System.out.println("Hərf: " + hərf);
        System.out.println("Mesaj: " + mesaj);
        System.out.println("Azadlıq: " + azadliq);
    }
}



Coffee coffee = new Coffee();
Developer developer = new Developer("Azərbaycanlı"); 

if (coffee.isEmpty()) {
    coffee.refill();
} else {
    developer.drink(coffee); 
}

Qəhvə qəhvə = new Qəhvə();
Proqramçı proqramçı = new Proqramçı("Azərbaycanlı");

if (qəhvə.boşdur()) {
    qəhvə.doldur();
} else {
    proqramçı.iç(qəhvə);
}



String test_ediləcək_veb_tətbiqin_url_i= "https://www.hamisiburada.az";

int ekrdna_obyektləri_gözləmə_vaxtı = 10; // saniyə 

System.out.println("Test ediləcək veb tətbiqin keçidi: " + url);


